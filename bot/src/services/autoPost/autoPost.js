...
async function postVtcRecruitment(client) {
    const channelIds = String(
        process.env.VTC_AUTOPOST_CHANNEL_IDS || ""
    )
        .split(",")
        .map(id => id.trim())
        .filter(Boolean);
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields(
            {
                name: "👥 Driver Organization",
                value: "Build and manage your trucking organization.",
                inline: true
            },
            {
                name: "🎮 ATS & ETS2",
                value: "Support for the trucking games your community loves.",
                inline: true
            },
            {
                name: "🌐 Community Growth",
                value: "Give your VTC a central home and grow your network.",
                inline: true
            }
        )
        .setFooter({
            text: "BC TRUCK WORKS • VTC Network"
        })
        .setTimestamp();
    ...
    for (const channelId of channelIds) {
        ...
        if (!channel || !channel.isTextBased()) {
            console.error(
                `[AUTOPOST] Invalid VTC channel configured: ${channelId}`
            );
        ...
        await channel.send({
            content:
                "🚨 **VTC OWNERS — YOUR NEXT STOP IS TRUCK WORKS!** 🚨",
            embeds: [embed],
    ...
    console.log(
        `[AUTOPOST] VTC recruitment posted to ${posted}/${channelIds.length} configured channel(s).`
    );
